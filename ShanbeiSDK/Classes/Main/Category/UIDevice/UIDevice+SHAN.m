...
+ (double)shan_systemVersion {
    ...
    dispatch_once(&onceToken, ^{
        version = [UIDevice shan_systemDetailVersion].doubleValue;
    });
    return version;
}
...
+ (NSString *)shan_systemDetailVersion {
    static NSString * detailVersion;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        detailVersion = [UIDevice currentDevice].systemVersion;
    });
...
+ (NSString *)shan_getDeviceId {
    NSString *deviceid = [[[UIDevice currentDevice].identifierForVendor.UUIDString stringByReplacingOccurrencesOfString:@"-" withString:@""] lowercaseString];
    return deviceid;
}
