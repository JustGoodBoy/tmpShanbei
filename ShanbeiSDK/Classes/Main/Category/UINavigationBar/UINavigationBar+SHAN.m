...
@implementation UINavigationBar (SHAN)
...
- (void)shan_navBarAlpha:(CGFloat)alpha isOpaque:(BOOL)opaque {
    ...
    if (!self.myNavView) {
        ...
        [self shan_navBarBottomLineHidden:self.hiddenBottom];
    }
...
- (void)shan_navBarBackGroundColor:(UIColor *)color image:(UIImage *)barImage isOpaque:(BOOL)opaque {
    [self clearSystemLayerIsOpaque:opaque];
    if (!self.myNavView) {
        //状态栏高度
        CGFloat statusHeight = [UIApplication sharedApplication].statusBarFrame.size.height;
    ...
    if (color) {
        self.myNavView.shanBackColor = color;
    }
    
    if (barImage) {
        self.myNavView.shanBackImage = barImage;
    }
...
// 设置图片背景颜色
- (void)setShanBackColor:(UIColor *)shanBackColor {
    _shanBackColor = shanBackColor;
    self.backImageView.backgroundColor = shanBackColor;
}

// 设置图片背景透明度
- (void)setShanAlpha:(CGFloat)shanAlpha {
    _shanAlpha = shanAlpha;
    self.backImageView.alpha = shanAlpha;
}

// 设置图片
- (void)setShanBackImage:(UIImage *)shanBackImage {
    _shanBackImage = shanBackImage;
    self.backImageView.image = shanBackImage;
}
